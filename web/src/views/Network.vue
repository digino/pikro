  ...
  <PageLayout title="Network" subtitle="Traffic overview">
    ...
    <div v-else class="grid gap-4">
      ...
      >
        ...
        <EmptyState v-else-if="logs.length === 0" message="No log entries" />
...
import { getPollSnapshot, getSystemLogs, getDHCPLeases } from '@/api'
...
function startTimers() {
  ...
  leasesTimer = setInterval(() => { if (!document.hidden) loadLeases() }, 30_000)
}
...
watch(() => store.activeId, async (id) => {
  ...
  leasesPage.value = 1
  if (!id) return
  await Promise.all([poll(), loadLogs(), loadLeases()])
  startTimers()
}, { immediate: true })
