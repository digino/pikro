<template>
  <PageLayout
    title="Speed Test"
    subtitle="Internet download speed measured from the router"
  >
    <NoRouterSelected v-if="!store.activeId" />

    <template v-else>
      <div class="border border-border rounded-xl p-5 max-w-xl bg-surface">
        <p class="text-text-secondary mb-4 leading-relaxed">
          The test runs
          <strong class="text-text-primary">from the router</strong> — not
          your browser. The router must have internet access to reach the
          target.
        </p>
        <div class="flex flex-col gap-1 mb-4">
          ...
          <p class="text-xs text-text-muted">
            Larger file = more accurate on fast connections
          </p>
        </div>
      ...
      <div v-if="result" class="flex flex-col gap-3 mt-4 max-w-md">
        <div class="border border-border rounded-xl p-5 text-center bg-surface">
          ...
          <p class="text-4xl font-bold text-text-primary">
            {{ formatSpeed(result["rx-speed"]) }}
          </p>
          <p class="text-xs text-text-muted mt-1">
            10 MB file · {{ result["duration"] || "—" }}
          </p>
        </div>
