      ...
      <template v-else>
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            >
              <td class="py-1.5 font-mono text-text-primary">{{ host['mac-address'] || '—' }}</td>
              <td class="py-1.5 font-mono text-text-primary">{{ host.address || '—' }}</td>
              <td class="py-1.5 text-text-secondary">{{ host.server || '—' }}</td>
              <td class="py-1.5">
                <StatusBadge
                  :color="host.authorized === 'true' ? 'green' : 'muted'"
                  :label="host.authorized === 'true' ? 'Authorized' : 'Unauthorized'"
                />
              </td>
              <td class="py-1.5 text-right font-mono text-text-secondary">{{ host.uptime || '—' }}</td>
              <td class="py-1.5 text-right font-mono text-text-muted">{{ host['idle-time'] || '—' }}</td>
            </tr>
          </tbody>
        </table>
        <div v-if="hosts.length > PAGE_SIZE" class="flex items-center justify-between pt-3 mt-1 border-t border-border">
          <span class="text-xs text-text-muted">
            {{ (page - 1) * PAGE_SIZE + 1 }}–{{ Math.min(page * PAGE_SIZE, hosts.length) }} of {{ hosts.length }}
          </span>
...
watch(() => store.activeId, async (id) => {
  ...
  if (!id) return
  await load()
  timer = setInterval(() => { if (!document.hidden) load() }, 5 * 60_000)
}, { immediate: true })
